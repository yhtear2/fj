...
public class CompInputProHandler implements Commandhandler {
	...
	public ModelAndView process(HttpServletRequest request, HttpServletResponse response) throws Throwable {
		...
		dto.setEmail("test@master");
		dto.setCeo(request.getParameter("ceo"));											// 대표자명
		dto.setComp_part(request.getParameter("comp_part"));								// 기업구분
		if( request.getParameter("tel_1").equals("직접입력")){									// 전화번호
			dto.setTel("직-"+request.getParameter("tel_2"));
		} else {
			dto.setTel(request.getParameter("tel_1") + "-" + request.getParameter("tel_2"));	
		}
